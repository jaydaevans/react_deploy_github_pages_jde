  ...
  return (
    ...
    <img src={props.student.image} alt="Student" className='card-img-top mx-auto'/>
      {!editMode && <ul className="list-group list-group-flush">
        ...
        <button type="button" className='btn btn-info' onClick={() => props.removeStudent(props.student)}>Delete <FontAwesomeIcon icon={faWarning} className='bg-transparent'/></button>
        <button type="button" className='btn btn-warning' onClick={() => setEditMode(true)}>Edit <FontAwesomeIcon icon={faMagicWandSparkles} className='bg-transparent'/></button>
      </ul>}
      {editMode &&
